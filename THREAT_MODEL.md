...
| `invoice_nft` | Mint as a non-compliant SME | When `set_risk_registry()` is configured, `mint_invoice`/`mint_invoices_batch` reject the SME (`ComplianceNotAttested`) *before* any invoice is stored. `marketplace.list_invoice` independently re-checks `compliance_attested` at listing time — see [docs/invoice-nft.md § Minting Rules](docs/invoice-nft.md#minting-rules) for why both are enforced |
...
6. **Mint-time compliance gating** — `invoice_nft` enforces `compliance_attested` at mint time (not just at `marketplace.list_invoice`), so a non-compliant SME cannot accumulate on-chain invoice metadata and events before being blocked
