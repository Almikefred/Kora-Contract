    ...
    pub fn fund_invoice(
        ...
        if fee > 0 {
            let referrer: Option<Address> = env
                .storage()
                .persistent()
                .get(&DataKey::Referrer(invoice_id));
            match referrer {
                Some(ref r) if config.referrer_split_bps > 0 => {
                    let referral_fee = bps_of_normalized(fee, config.referrer_split_bps, token_decimals)?;
                    let treasury_fee = fee
                        .checked_sub(referral_fee)
                        .ok_or(KoraError::ArithmeticOverflow)?;
                    if referral_fee > 0 {
                        token_client.transfer(&investor, r, &referral_fee);
                    }
                    if treasury_fee > 0 {
                        token_client.transfer(&investor, &config.treasury, &treasury_fee);
                    }
                    events::referral_fee_paid(&env, invoice_id, r, referral_fee, treasury_fee);
                }
                _ => {
